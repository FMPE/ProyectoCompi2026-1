...
// -----------------------------------------------------------------------------
// Recolección de llamadas vivas (no plegadas) para eliminar funciones muertas.
// -----------------------------------------------------------------------------
static void collectCallsE(Exp *e, std::set<std::string> &s);
static void collectCallsS(Stm *st, std::set<std::string> &s);
static void collectCallsB(Body *b, std::set<std::string> &s);

static void collectCallsE(Exp *e, std::set<std::string> &s) {
  if (!e)
    return;
  if (auto b = dynamic_cast<BinaryExp *>(e)) {
    collectCallsE(b->left, s);
    collectCallsE(b->right, s);
  } else if (auto u = dynamic_cast<UnaryExp *>(e)) {
    collectCallsE(u->operand, s);
  } else if (auto f = dynamic_cast<FcallExp *>(e)) {
    if (!f->isConstant) {
      s.insert(f->nombre);
      for (auto a : f->argumentos)
        collectCallsE(a, s);
    }
  } else if (auto ix = dynamic_cast<IndexExp *>(e)) {
    collectCallsE(ix->index, s);
  } else if (auto m = dynamic_cast<MatrixExp *>(e)) {
    collectCallsE(m->row, s);
    collectCallsE(m->col, s);
  } else if (auto ls = dynamic_cast<ExpListSize *>(e)) {
    collectCallsE(ls->size, s);
  } else if (auto lv = dynamic_cast<ExpListVals *>(e)) {
    for (auto v : lv->values)
      collectCallsE(v, s);
  } else if (auto ms = dynamic_cast<ExpMatrixSize *>(e)) {
    collectCallsE(ms->rows, s);
    collectCallsE(ms->cols, s);
  } else if (auto mv = dynamic_cast<ExpMatrixVals *>(e)) {
    collectCallsE(mv->rows, s);
    collectCallsE(mv->cols, s);
    for (auto v : mv->values)
      collectCallsE(v, s);
  }
  // NumberExp, IdExp, FieldExp: sin llamadas.
}

static void collectCallsS(Stm *st, std::set<std::string> &s) {
  if (!st)
    return;
  if (auto a = dynamic_cast<AssignStm *>(st)) {
    collectCallsE(a->target, s);
    collectCallsE(a->e, s);
  } else if (auto p = dynamic_cast<PrintStm *>(st)) {
    collectCallsE(p->e, s);
  } else if (auto r = dynamic_cast<ReturnStm *>(st)) {
    collectCallsE(r->e, s);
  } else if (auto i = dynamic_cast<IfStm *>(st)) {
    collectCallsE(i->condition, s);
    collectCallsB(i->then, s);
    if (i->els)
      collectCallsB(i->els, s);
  } else if (auto w = dynamic_cast<WhileStm *>(st)) {
    collectCallsE(w->condition, s);
    collectCallsB(w->b, s);
  } else if (auto d = dynamic_cast<DoWhileStm *>(st)) {
    collectCallsE(d->condition, s);
    collectCallsB(d->b, s);
  } else if (auto sw = dynamic_cast<SwitchStm *>(st)) {
    collectCallsE(sw->e, s);
    for (auto c : sw->cases)
      for (auto cs : c->body)
        collectCallsS(cs, s);
    for (auto ds : sw->default_body)
      collectCallsS(ds, s);
  }
  // BreakStm: nada.
}
...
int GenCodeVisitor::generar(Program *program) {
  ...
  std::unordered_map<std::string, FunDec *> fdMap;
  for (auto fd : program->fdlist) {
  ...
  while (!worklist.empty()) {
    ...
    worklist.pop_back();
    if (liveFunctions.count(n))
      continue;
    liveFunctions.insert(n);
    auto it = fdMap.find(n);
    if (it == fdMap.end())
      continue;
    std::set<std::string> calls;
    collectCallsB(it->second->cuerpo, calls);
    for (auto &c : calls)
...
int GenCodeVisitor::visit(Program *program) {
  ...
  out << "\n.text\n";
  for (auto fd : program->fdlist) {
    if (!liveFunctions.count(fd->nombre))
      continue;
    fd->accept(this);
  }
...
int GenCodeVisitor::visit(BinaryExp *exp) {
  // Subexpresión completamente constante: plegada por Opt1.
  if (exp->isConstant) {
    out << "  movq $" << exp->constantValue << ", %rax\n";
    return 0;
  }

  auto esPotenciaDe2 = [](int e) { return e >= 1 && (e & (e - 1)) == 0; };
  if (exp->op == POW_OP && exp->right->isConstant &&
      esPotenciaDe2(exp->right->constantValue)) {
    int e = exp->right->constantValue;
    int k = 0;
    while ((1 << k) < e)
      k++; // k = log2(e)
    exp->left->accept(this);
...
int GenCodeVisitor::visit(FcallExp *exp) {
  // Llamada plegada a constante por Opt1.
  if (exp->isConstant) {
    out << "  movq $" << exp->constantValue << ", %rax\n";
    return 0;
  }
...
// Evalúa una expresión en tiempo de compilación. Devuelve true y rellena res
// si toda la expresión es constante (incluyendo llamadas a funciones plegables).
bool Opt1Visitor::tryEval(Exp *e, std::unordered_map<std::string, int> &env,
                          int &res) {
  ...
  if (auto b = dynamic_cast<BinaryExp *>(e)) {
    int l, r;
    if (!tryEval(b->left, env, l) || !tryEval(b->right, env, r))
      return false;
    switch (b->op) {
    case PLUS_OP:  res = l + r; return true;
    case MINUS_OP: res = l - r; return true;
    case MUL_OP:   res = l * r; return true;
    case DIV_OP:   if (r == 0) return false; res = l / r; return true;
    case POW_OP: {
      int acc = 1;
      for (int i = 0; i < r; ++i)
        acc *= l;
    ...
    case LE_OP:  res = (l <  r); return true;
    case GT_OP:  res = (l >  r); return true;
    case LEQ_OP: res = (l <= r); return true;
    case GEQ_OP: res = (l >= r); return true;
    case EQ_OP:  res = (l == r); return true;
    case NE_OP:  res = (l != r); return true;
    case AND_OP: res = (l && r); return true;
    case OR_OP:  res = (l || r); return true;
    }
    return false;
  }
  if (auto fc = dynamic_cast<FcallExp *>(e)) {
    auto it = foldable.find(fc->nombre);
    if (it == foldable.end())
      return false;
    FunDec *fd = it->second;
    if (fc->argumentos.size() != fd->Pnombres.size())
      return false;
    std::unordered_map<std::string, int> env2;
    for (size_t i = 0; i < fc->argumentos.size(); ++i) {
      int av;
      if (!tryEval(fc->argumentos[i], env, av))
        return false;
      env2[fd->Pnombres[i]] = av;
    }
    ReturnStm *ret = dynamic_cast<ReturnStm *>(fd->cuerpo->StmList.front());
    return tryEval(ret->e, env2, res);
  }
  return false;
}
...
{
  // Detectar funciones plegables: cuerpo = un único return(expr), sin
  // declaraciones locales.
  for (auto fd : program->fdlist) {
    if (fd->cuerpo->declarations.empty() && fd->cuerpo->StmList.size() == 1 &&
        dynamic_cast<ReturnStm *>(fd->cuerpo->StmList.front()))
      foldable[fd->nombre] = fd;
  }
  program->accept(this);
  return 0;
}
...
int Opt1Visitor::visit(BinaryExp *exp)
{
    ...
    if (exp->isConstant)
    {
      ...
      case DIV_OP:
      {
       if (exp->right->constantValue == 0) { exp->isConstant = false; break; }
       exp->constantValue = exp->left->constantValue / exp->right->constantValue;
       break;}
      case POW_OP:
      {
       int acc = 1;
       for (int i = 0; i < exp->right->constantValue; ++i)
         acc *= exp->left->constantValue;
       exp->constantValue = acc;
       break;}
      default:
       // Operadores relacionales/lógicos: no se pliegan aquí.

...
public:
  // Funciones "plegables": cuerpo = un único return(expr) sin declaraciones.
  std::unordered_map<std::string, FunDec *> foldable;
  // Evalúa una expresión en tiempo de compilación si es posible.
  bool tryEval(Exp *e, std::unordered_map<std::string, int> &env, int &res);
...
public:
  ...
  bool needsPotencia = false;    // ¿Se generó alguna llamada a 'potencia'?
  std::set<std::string> liveFunctions; // Funciones alcanzables desde main

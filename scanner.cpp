...
Token* Scanner::nextToken() {
    ...
    if (isalpha(c) || c == '_') {
        ...
        if (lexema == "as") return new Token(Token::AS, lexema, 0, (int)lexema.size());
        // Tipos primitivos
        ...
        if (lexema == "String") return new Token(Token::STRING, lexema, 0, (int)lexema.size());
    ...
    if (c == ':' && peek(1) == ':') { current += 2; return new Token(Token::COLONCOLON, "::", 0, 2); }

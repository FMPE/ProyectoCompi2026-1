...
import os, re, subprocess, json, csv, platform
...
BENCH_MD = os.path.join(ROOT, "benchmarks", "results.md")
...
def parse_asm_size_table(md_path):
    """Extrae la tabla '## Tamaño del código x86 generado' de benchmarks/results.md.

    Cada celda tiene forma 'instr / total' o 'N/A'; nos quedamos con el primer
    número (instrucciones de la función), que es la métrica comparable entre
    compiladores.
    """
    ...
    section = re.search(r"## Tamaño del código x86 generado.*?\n(.*?)(?:\n##|\Z)", text, re.S)
    if not section:
        return []
    ...
    for line in section.group(1).splitlines():
        ...
        if not m:
            continue
        name, func, ours, llvm, clif, gcc = m.groups()

        def first_num(cell):
            mm = cell_re.search(cell)
...
def try_matplotlib_charts(opt_rows):
    ...
    # Tamaño de código x86 (instrucciones por función) si la tabla existe en results.md
    size_rows = parse_asm_size_table(BENCH_MD)
    if size_rows:
        fig3, ax3 = plt.subplots(figsize=(9, 4.5))
        progs = [r["program"] for r in size_rows]
        x = range(len(progs))
        w = 0.2
        series = [
            ("Nuestro", "ours", "#2563eb"),
            ("Rustc/LLVM", "llvm", "#dc2626"),
            ("Rustc/Cranelift", "clif", "#a855f7"),
            ("GCC", "gcc", "#16a34a"),
        ]
        offsets = [-1.5 * w, -0.5 * w, 0.5 * w, 1.5 * w]
        for (label, key, color), off in zip(series, offsets):
            vals = [r[key] if r[key] is not None else 0 for r in size_rows]
            ax3.bar([i + off for i in x], vals, w, label=label, color=color)
        ax3.set_xticks(list(x))
        ax3.set_xticklabels(progs)
        ax3.set_ylabel("Instrucciones (función)")
        ax3.set_title("Tamaño del código x86 generado (instrucciones por función)")
        ax3.legend()
        fig3.tight_layout()
        p3 = os.path.join(ASSETS, "grafico_tamano_codigo.png")
...
- Gráficos: `grafico_optimizacion.png`, `grafico_benchmark.png` y `grafico_tamano_codigo.png`
""")
    print("OK", path)
